...
public:
    void lock() {
        // Ждем пока флаг locked станет false, затем устанавливаем его в true
        bool expected = false;
        while (!locked.compare_exchange_weak(expected, true, std::memory_order_acquire)) {
            expected = false;  // В случае неудачи сбрасываем expected
            std::this_thread::yield();  // Даем возможность другим потокам
        }
    }

    void unlock() {
        // Устанавливаем флаг locked в false
        locked.store(false, std::memory_order_release);
    }
};

// ============================ Основная программа ============================
CustomMutex kitchen_lock;  // Используем собственный мьютекс вместо std::mutex
...
void chef_routine(int productivity) {
    while (simulation_active && !scenario_finished) {
        ...
        // Используем собственный мьютекс
        kitchen_lock.lock();
...
int main() {
    ...
    std::cout << "ЛАБОРАТОРНАЯ РАБОТА №5: ТРИ ТОЛСТЯКА" << std::endl;
    std::cout << "Собственная реализация мьютекса на основе compare_exchange" << std::endl;
    std::cout << "==========================================================" << std::endl;
    ...
    std::cout << "\n==========================================================" << std::endl;
    std::cout << "Все сценарии завершены. Демонстрация работы пользовательского мьютекса:" << std::endl;
    std::cout << "1. Использует атомарную переменную и compare_exchange_weak" << std::endl;
    std::cout << "2. При неудачной попытке захвата вызывает yield()" << std::endl;
    std::cout << "3. Гарантирует mutual exclusion как стандартный мьютекс" << std::endl;
